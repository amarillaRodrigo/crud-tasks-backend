...
getAllTasks: async (req, res) => {
    try {
        ...
        if (result.length === 0) {
            res.status(404).json({ msg: 'La tarea no está registrada en la base de datos' });
        } else {
...
createTask : async (req, res) => {
    ...
    if (!title || typeof title !== 'string' || title.length === 0 || title.length > 255) {
        return res.status(400).json({ msg: 'El título no puede ser una cadena vacia y no debe superar los 255 caracteres.' });
    }

    if (!description || typeof description !== 'string' || description.length === 0) {
        return res.status(400).json({ msg: 'La descripción no puede ser una cadena vacia.' });
    }

    if (typeof isComplete !== 'boolean') {
        return res.status(400).json({ msg: 'El estado de la tarea debe ser un valor booleano.' });
    }
...
updateTask : async (req, res) => {
    ...
    try {
        const conexion = await connection;
        const [taskFound] = await conexion.query('SELECT * FROM tasks WHERE id = ?', [id]);
        if (taskFound.length === 0) {
            res.status(404).json({ msg: 'Tarea no encontrada' });
        } else {
            const [result] = await conexion.query('UPDATE tasks SET title = ?, description = ?, isComplete = ? WHERE id = ?', [title, description, isComplete, id]);
            if (!result.ok) {
